...
public class Export {
	...
	public static String writeCsvToSD(List<Gps> list, boolean dataSwitch) {
		...
		try {
			String pathName = "/sdcard/weng/";
			String fileName = dataSwitch ? "wild_research_poi_" + df.format(new Date()) + ".csv" : "wild_research_tack_" + df.format(new Date()) + ".csv";
	...
	public static String writeXMLToSD(List<Gps> list, boolean dataSwitch) {
		...
		try {
			...
			String fileName = dataSwitch ? "wild_research_poi_" + df.format(new Date()) + ".xml" : "wild_research_tack_" + df.format(new Date()) + ".xml";
			File path = new File(pathName);
			File file = new File(pathName + fileName);
			if (!path.exists()) {
				Log.d("TestFile", "Create the path:" + pathName);
